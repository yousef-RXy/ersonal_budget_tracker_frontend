...
export default function myBudget() {
	...
	const handleOptimisticSubmit = (data) => {
		...
		const updatedItem = {
			...
			name: data.name,
			repetitionPeriod: data.repetitionPeriod || "None",
		};
	...
	return (
		<div className="bg-[#141935] min-h-full min-w-full p-8 flex justify-center gap-8">
			...
			<div className="flex flex-col gap-8">
				...
				>
					<select
						className="mb-2 p-2 rounded bg-[#1b2055] shadow-2xl"
						value={incomeCategoryFilter}
						onChange={(e) => setIncomeCategoryFilter(e.target.value)}
					>
						<option value="all">All Categories</option>
						{[...new Set(income.map((i) => i.category))].map((category) => (
							<option
								key={category}
								value={category}
							>
					...
					{income
						.filter(
							(i) =>
								...
								i.category === incomeCategoryFilter
						)
						.map((i) => (
							<div
								key={i.id}
			...
			<div className="flex flex-col gap-8">
				...
				>
					...
					>
						<option value="all">All Categories</option>
						{[...new Set(expense.map((i) => i.category))].map((category) => (
							<option
								key={category}
								value={category}
							>
					...
					{expense
						.filter(
							(i) =>
								...
								i.category === expenseCategoryFilter
						)
						.map((i) => (
...
export async function clientAction({ request }) {
	...
	const { isUpdate, type, repetitionPeriod, ...rest } = data;
	const userId = localStorage.getItem("id");
	const jwt = localStorage.getItem("jwt");
	...
	rest.userId = userId;
	rest.repetitionPeriod = repetitionPeriod || "None";
